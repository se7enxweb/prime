...
class Response
{
    ...
    {
        $contentDisposition = (string) $this->headers->get('Content-Disposition');

        if (false !== stripos($contentDisposition, 'attachment') && 1 == preg_match('/MSIE (.*?);/i', $request->server->get('HTTP_USER_AGENT'), $match) && true === $request->isSecure()) {
